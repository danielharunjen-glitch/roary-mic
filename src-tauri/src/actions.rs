...
pub(crate) async fn process_transcription_output(
    ...
    // Apply user-authored corrections (learned from history edits) as a last
    // pass before paste. Any transformation these make is reflected in
    // post_processed_text so the history reflects what was actually pasted.
    if let Some(hm) = app.try_state::<Arc<HistoryManager>>() {
        match hm.get_active_corrections() {
            Ok(corrections) if !corrections.is_empty() => {
                let rewritten =
                    crate::managers::history::apply_corrections(&final_text, &corrections);
                if rewritten != final_text {
                    debug!("Applied {} corrections to transcription", corrections.len());
                    final_text = rewritten;
                    post_processed_text = Some(final_text.clone());
                }
            }
            Ok(_) => {}
            Err(e) => error!("Failed to load corrections: {}", e),
        }
    }
...
async fn handle_ai_mode_completion(
    ah: &AppHandle,
    ...
    if transcription.trim().is_empty() {
        if wav_saved {
            if let Err(e) = hm.save_entry(file_name, String::new(), true, None, None) {
                error!("Failed to save empty AI-mode entry: {}", e);
            }
        }
    ...
    let provider = match settings
        .post_process_provider(&settings.ai_mode_provider_id)
        .cloned()
    {
        Some(p) => p,
        None => {
            error!(
                "AI mode: provider '{}' not found in configured providers",
                settings.ai_mode_provider_id
            );
            let _ = ah.emit("ai-mode-error", "AI mode provider is not configured.");
            utils::hide_recording_overlay(ah);
            change_tray_icon(ah, TrayIconState::Idle);
            return;
        }
    ...
    let system_prompt = if settings.ai_mode_prompt.trim().is_empty() {
        None
    } else {
        Some(settings.ai_mode_prompt.clone())
    };

    let reply = match crate::llm_client::send_chat_completion_multimodal(
        &provider,
        api_key,
        &settings.ai_mode_model,
        system_prompt,
        transcription.clone(),
    ...
    .await
    {
        Ok(Some(text)) => text,
        Ok(None) => {
            warn!("AI mode: provider returned empty response");
            String::new()
        }
        Err(e) => {
            error!("AI mode request failed: {}", e);
            let _ = ah.emit("ai-mode-error", e);
            utils::hide_recording_overlay(ah);
            change_tray_icon(ah, TrayIconState::Idle);
            return;
        }
    ...
    let reply = strip_invisible_chars(reply.trim());
    ...
    if wav_saved {
        if let Err(e) = hm.save_entry(
            ...
            error!("Failed to save AI-mode history entry: {}", e);
        }
    }
