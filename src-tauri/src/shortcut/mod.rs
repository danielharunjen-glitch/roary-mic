...
fn register_all_shortcuts_for_implementation(
    ...
    for (id, default_binding) in &default_bindings {
        ...
        // Skip AI-mode shortcut when the feature is disabled
        if id == "ai_mode" && !current_settings.ai_mode_enabled {
            continue;
        }
...
#[tauri::command]
#[specta::specta]
pub fn change_ai_mode_enabled_setting(app: AppHandle, enabled: bool) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.ai_mode_enabled = enabled;
    settings::write_settings(&app, settings.clone());

    if let Some(binding) = settings.bindings.get("ai_mode").cloned() {
        if enabled {
            let _ = register_shortcut(&app, binding);
        } else {
            let _ = unregister_shortcut(&app, binding);
...
#[tauri::command]
#[specta::specta]
pub fn change_ai_mode_provider_setting(app: AppHandle, provider_id: String) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    validate_provider_exists(&settings, &provider_id)?;
    settings.ai_mode_provider_id = provider_id;
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub fn change_ai_mode_model_setting(app: AppHandle, model: String) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.ai_mode_model = model;
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub fn change_ai_mode_prompt_setting(app: AppHandle, prompt: String) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.ai_mode_prompt = prompt;
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub fn change_ai_mode_include_screenshot_setting(
    app: AppHandle,
    enabled: bool,
) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.ai_mode_include_screenshot = enabled;
    settings::write_settings(&app, settings);
    Ok(())
}
