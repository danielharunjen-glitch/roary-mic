...
pub struct AppSettings {
    ...
    #[serde(default = "default_ai_mode_provider_id")]
    pub ai_mode_provider_id: String,
    #[serde(default)]
    pub ai_mode_model: String,
    #[serde(default = "default_ai_mode_prompt")]
...
fn default_ai_mode_provider_id() -> String {
    "anthropic".to_string()
}

fn default_ai_mode_prompt() -> String {
    "You are a voice-controlled writing assistant. The user just dictated a request \
     and (optionally) you can see a screenshot of what's on their screen. \
     Respond with ONLY the exact text the user wants inserted into the active \
     application. No preamble, no quotes, no commentary — just the text to paste."
        .to_string()
}
...
pub fn get_default_settings() -> AppSettings {
    ...
    #[cfg(target_os = "windows")]
    let default_ai_mode_shortcut = "ctrl+alt+space";
    #[cfg(target_os = "macos")]
    let default_ai_mode_shortcut = "cmd+shift+space";
    #[cfg(target_os = "linux")]
    let default_ai_mode_shortcut = "ctrl+alt+space";
    #[cfg(not(any(target_os = "windows", target_os = "macos", target_os = "linux")))]
    let default_ai_mode_shortcut = "alt+shift+space";

    bindings.insert(
        "ai_mode".to_string(),
        ShortcutBinding {
            id: "ai_mode".to_string(),
            name: "AI Mode".to_string(),
            description:
                "Sends your voice request (and optionally a screenshot) to an LLM and pastes the reply."
                    .to_string(),
            default_binding: default_ai_mode_shortcut.to_string(),
            current_binding: default_ai_mode_shortcut.to_string(),
        },
    );
    ...
    AppSettings {
        ...
        ai_mode_provider_id: default_ai_mode_provider_id(),
